...
DATAdiagnostics <- function(object) {
  ...
  # if(all(rownames(dat) != object@Metadata$Samples)) stop('Samples are out of order.')
  ...
  feature_info$fc.notes <- paste0(levels(object@Metadata$Condition)[2], ' over ', levels(object@Metadata$Condition)[1])
  feature_info$statistic <- sapply(1:num_features, function(i) t.test(separated_conditions_data[[2]][i, ], separated_conditions_data[[1]][i, ], var.equal = FALSE)$statistic)
  feature_info$pvalue <- sapply(1:num_features, function(i) t.test(separated_conditions_data[[2]][i, ], separated_conditions_data[[1]][i, ], var.equal = FALSE)$p.value)

...
If you elect to optimize the $\lambda$ parameter using a different method than `BICtune()`, you can specify the value to use with the `optimal_lambda` parameter. Only 50 of our 322 samples are "DM:case", therefore, we will set `nreps=1000` and `subSample = TRUE`. This is the most computationally expensive step in the algorithm. For the example data, the workflow up to this point took ~15 minutes using 4 cores on a 2.5 GHz Quad-Core Intel Core i7 processor.

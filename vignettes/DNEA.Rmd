...
## Feature aggregation
As mentioned previously, glasso models, and therefore DNEA, are computationally expensive algorithms to perform. Adding additional samples to your dataset will have marginal effects on the memory and processing time required for analysis, however, as the number of features grows in your dataset the runtime will increase dramatically. The implementation of this R package and the ability to utilize the full resources of a high-performance machine (cluster or cloud resource) to perform stability selection embarassingly parallel helps tremendously with this issue.  
...
Feature reduction is an optional step in the DNEA algorithm and the user may decide to completely skip this step - particulary when their dataset contains a diverse set of features and ***p >> n***. Should you decide to collapse correlated features prior to analysis, we have provided the `reduceFeatures()` function to do. This function will collapse the features based on the user specifications and re-run differential expression analysis and diagnostic testing before creating a `collapsed_DNEAresults` object with the new data.  

To perform feature collapsing, we need to provide the algorithm with the DNEAresults object and a character string corresponding to the method used. If the "correlation" or "hybrid" method are chosen, we need to specify the `correlation threshold` parameter wherein metabolites above this threshold are collapsed into a single node. If the "knowledge" or "hybrid" method are chosen, we need to provide a data frame for `feature_groups` to specify the class of molecules to collapse within. More information about the three methods can be found in the function documentation accessed by running `?reduceFeatures()` in the console.  

In this example, we're going to use the "hybrid" method and specify a `correlation_threshold` of 0.7.
```{r setup input for reduceFeatures, eval=TRUE}
#create molecule class labels for the metabolites
TEDDY_groups <- data.frame(features = rownames(expressionData(TEDDYdat, normalized = FALSE)),
                           groups = rownames(expressionData(TEDDYdat, normalized = FALSE)),
                           row.names = rownames(expressionData(TEDDYdat, normalized = FALSE)))
...
TEDDY_groups[1:10, ]
```
As you can see, the `feature_groups` input should be a data frame with the original feature names as the row names as well as the "features" column, and the class labels should be in the "groups" column. Independent molecules can retain their name. 
...
collapsed_TEDDY
```
The output looks very similar to that of `createDNEAobject()`. Additionally, we see from the summary that we have reduced our total number of features from 134 to 130 using `reduceFeatures()`.

# STEP 2: Model Optimization
Model optimization contains two regularization methods: $\lambda$ tuning via Bayesian-information criterion (BIC), and stability selection. Depending on the dataset, it may be skipped but we highly recommend the user performs this step irrespective. Only in cases where **the number of features approaches or exceeds the number of samples** can this step be skipped. The $\lambda$ value in the model then defaults to $\lambda = \sqrt{ \ln (num. features) / num. samples}$ and all of the selection probabilities are set to 1.  

## $\lambda$ tuning via BIC
The `BICtune()` function optimizes the $\lambda$ parameter by calculating a BIC score and likelihood value for every tested value. We minimized the necessary computation to optimize the parameter by selecting values surrounding the asymptotically valid $\lambda$ for datasets with many samples and many features followin the equation: \lambda = c \sqrt{ \ln (num. features) / num. samples} where c takes on 15 evenly spaced values between 0.01 and 0.3. The user can also opt to provide a range of lambda values to test using the `lambda_values` parameter.  


Each $\lambda$ value tested can be run embarrasingly parallel using the BiocParallel package. Creating Forked processes via the `MulticoreParam()` function is the most resource efficient parallelization method, however, there are several limitations: **1.** forked processes are not available on the Windows operating system, and **2.** forked processes can be unstable in Rstudio. For the sake of this vignette we're going to create parallel sockets. 
```{r BiocParallel, eval=TRUE}
#load in BiocParallel
library(BiocParallel)

#create parallel workers
BPPARAM <- BiocParallel::SnowParam(RNGseed = 417)
```
We only need to create the workers once for this analysis, and they can be used for stability selection. However, `stabilitySelection()` uses random number generation to sample the expression data, so we'll have to set the seed now so we can reproduce the results.  


Now that we have our workers set up, we can optimize $\lambda$
```{r BICtune, eval=TRUE, echo=FALSE}

#optimize lambda
TEDDYdat <- BICtune(TEDDYdat, BPPARAM = BPPARAM)
TEDDYdat
```
